...
import org.springframework.security.access.AccessDeniedException; // 🎯 EKLENDİ: IDOR muhafızı için
...
public class OrderService {
    ...
    private void assignPortToCustomer(Customer customer, InfrastructureNode node) {
        ...
        Port port = new Port();
        port.setPortNumber(assignedPortNumber);
        ...
        log.info("🔌 [AKILLI PORT ATAMA] Müşteri '{}' için fiziksel Port (No: {}) saha dolabına ({}) mermer gibi çakıldı!",
    ...
    public void processOrderFromQueue(Long orderId) {
        ...
        if (building == null) {
            updateOrderStatus(order, "IPTAL_EDILDI", "Bina lokasyon verisi bulunamadığı için sipariş iptal edildi.");
            return;
        }

        InfrastructureNode closestNode = nodeRepository.findClosestNode(building.getLocation()).orElse(null);
        if (closestNode == null) {
            updateOrderStatus(order, "IPTAL_EDILDI", "Binaya hizmet verebilecek menzilde altyapı saha dolabı bulunamadı.");
            return;
        }
    ...
    public List<OrderStatusHistory> getOrderHistory(Long orderId) {
        ...
        // 🎯 YENİ MUHAFIZ: SecurityException yerine AccessDeniedException fırlatıyor!
        if (!isAdmin && !order.getCustomer().getId().equals(currentCustomer.getId())) {
            throw new AccessDeniedException("Ağ Güvenlik Protokolü: Bu sipariş tarihçesini görüntüleme yetkiniz bulunmamaktadır!");
        }
    ...
    public String cancelOrder(Long orderId) {
        ...
        if (order.getCustomer() == null || !order.getCustomer().getEmail().equals(currentUsersEmail)) {
            ...
            throw new AccessDeniedException("Ağ Güvenlik Protokolü: Bu siparişi iptal etme yetkiniz bulunmuyor!");
        }
